...
# -*- coding: utf-8 -*-
"""
Medication Voice Assistant — DIAG BUILD (ASR bytes fix)
- Fixes Vosk+sounddevice callback TypeError by feeding bytes(indata) to AcceptWaveform.
- Keeps Pillow 10+ safe text rendering and PiTFT pin mapping (cs=D5, dc=D25, rst=None).
- Adds extra logs around button callbacks and ASR device selection.

Run (buttons only, no display):
  USE_DISPLAY=0 BUTTON_BACKEND=gpio GPIOZERO_PIN_FACTORY=lgpio \
  \
  "../.venv/bin/python" med_assistant_v3_diag_fix.py

Then try with display (ensure piscreen.service stopped):
  sudo systemctl stop piscreen.service --now
  BUTTON_BACKEND=gpio USE_DISPLAY=1 GPIOZERO_PIN_FACTORY=lgpio \
  ../.venv/bin/python med_assistant_v3_diag_fix.py

Optional env to auto-pick your mic by name substring (e.g., "Logi"):
  SD_INPUT_NAME=Logi 
"""
...
_DISPLAY_AVAILABLE = _ASR_AVAILABLE and (_USE_DISPLAY)  # only try display if ASR stack imported fine
try:
...
BUTTON_BACKEND = os.getenv('BUTTON_BACKEND', '').strip().lower()
...
def humanize_delta(seconds:int) -> str:
    ...
    m, s = divmod(rem, 60)
    if h:
        if m: return f"{h} hours {m} minutes" if h!=1 else f"{h} hour {m} minutes"
        return f"{h} hours" if h!=1 else "1 hour"
    if m: return f"{m} minutes" if m!=1 else "1 minute"
...
class Speaker:
    ...
    def say(self, text:str, rate_wpm:int=170):
        ...
        print('[TTS]', text)
        if self._has_espeak:
            try: subprocess.run(['espeak-ng','-s',str(rate_wpm), text], check=False)
            except Exception as e: print('[TTS] espeak-ng error:', e)
...
class Display:
    ...
    def _measure(self, text:str):
        if hasattr(self._draw,'textbbox'):
            box = self._draw.textbbox((0,0), text, font=self._font)
    ...
    def _write_center(self, lines, fill=(255,255,255)):
        ...
        for (ln,(w,h)) in zip(lines,sizes):
            ...
            y+=h+2
        self._disp.image(self._image, rotation=self.ROTATION)
    def show_idle(self): self._write_center(['Medication Assistant','Press TOP to start'], (180,220,255))
    def show_listening(self): self._write_center(['Listening...','Say a time'], (255,255,180))
    ...
    def show_ringing(self): self._write_center(['Time to take','medicine'], (255,200,200))
    def show_snoozed(self): self._write_center(['Snoozed','30 s'], (200,220,255))
    def show_finished(self): self._write_center(['Taken','OK'], (200,255,200))
    def show_error(self, msg:str): self._write_center(['Error', msg or ''], (255,180,180))
...
class Listener:
            ...
            try:
                ...
                for i, d in enumerate(devs):
                    if d.get('max_input_channels',0) > 0 and name_hint.lower() in str(d.get('name','')).lower():
                        ...
                        print(f"[ASR] Using input device #{i}: {d['name']}")
                        break
            except Exception as e:
    ...
    def listen(self, max_sec:int) -> str | None:
        ...
        def cb(indata, frames, time_info, status):
            ...
            try:
                # IMPORTANT: feed bytes to Vosk (fixes cffi TypeError on some builds)
                if rec.AcceptWaveform(bytes(indata)):
                    result_text=_extract_text(rec.Result())
            except Exception as e:
                # Log but do not crash the whole app
...
def parse_time_expr(utter:str):
    ...
    if dateparser is not None:
        dp=dateparser.parse(utter, languages=['en'], settings={'PREFER_DATES_FROM':'future','RELATIVE_BASE':now,'RETURN_AS_TIMEZONE_AWARE':bool(LOCAL_TZ),'TIMEZONE':os.getenv('TZ')})
        if dp:
...
def _parse_rel(text:str)->int|None:
    ...
    p=re.compile(r"(\d+)\s*(h|hr|hrs|min|m|sec|s)", re.I)
    total=0; found=False
    ...
    def _try_gpio(self) -> bool:
        ...
        try:
            ...
            if DEBUG:
                ...
                print("[GPIO] Initial states: top.is_pressed=", self._btn_top.is_pressed, " bottom.is_pressed=", self._btn_bottom.is_pressed)
            def cb_top():
                print('[CB] gpiozero TOP when_pressed fired')
                self.on_top()
            def cb_bottom():
                print('[CB] gpiozero BOTTOM when_pressed fired')
                self.on_bottom()
            self._btn_top.when_pressed = cb_top
            self._btn_bottom.when_pressed = cb_bottom
            return True
        except Exception as e:
            ...
            try:
                ...
                for ev in dev.read_loop():
                    if ev.type == ecodes.EV_KEY and ev.value == 1:
                        if DEBUG: print('[EVDEV]', p, 'code=', ev.code)
                        if ev.code in (ecodes.KEY_UP, ecodes.KEY_ENTER, ecodes.KEY_VOLUMEUP, ecodes.KEY_KPENTER):
                            print('[CB] evdev TOP fired'); self.on_top()
                        elif ev.code in (ecodes.KEY_DOWN, ecodes.KEY_VOLUMEDOWN, ecodes.KEY_PAGEDOWN, ecodes.KEY_NEXT):
                            print('[CB] evdev BOTTOM fired'); self.on_bottom()
            except Exception as e:
...
class Assistant:
    ...
    def run(self):
        print('=== Medication Voice Assistant (diag-fix) ===')
    ...
    def _on_bottom(self):
        if self.state==self.RINGING:
            ...
            self.display.show_finished(); self.schedule_dt=None; self.state=self.IDLE; self.display.show_idle()
    ...
    def _on_window_timeout(self):
        if self.state==self.RINGING:
            print('[STATE] Auto-snooze 30s'); self.state=self.SNOOZE
            threading.Thread(target=lambda:(time.sleep(SNOOZE_SEC), self.event_q.put(('SNOOZE_TIMEOUT', None))), daemon=True).start()
    ...
    def _capture_and_schedule(self):
        ...
        self.schedule_dt=dt
        if re.search(r"\b(in|after)\b", (utter or ''), re.I): self.speaker.say(f"Got it. I'll remind you in {delta}.")
        else:
            at_str = dt.astimezone(ZoneInfo(os.getenv('TZ','America/New_York'))).strftime('%I:%M %p').lstrip('0') if (ZoneInfo and dt.tzinfo) else dt.strftime('%H:%M')
        ...
        def waiter():
            while True:
                ...
                if now_tz() >= dt: self.event_q.put(('TIME_REACHED', None)); return
                time.sleep(0.2)
...
if __name__=='__main__':
    ...
    print('GPIOZERO_PIN_FACTORY:', os.getenv('GPIOZERO_PIN_FACTORY','<default>'))
    print('----------------------------------')
