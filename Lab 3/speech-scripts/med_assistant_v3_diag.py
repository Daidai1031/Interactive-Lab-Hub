...
# -*- coding: utf-8 -*-
"""
Medication Voice Assistant — DIAG BUILD
- Adds immediate debug prints inside GPIO button callbacks so we can see
  whether gpiozero receives the edge at all.
...
Run (buttons only, no display):
  ...
  \
  "../.venv/bin/python" med_assistant_v3_diag.py
...
class Listener:
    ...
    def listen(self, max_sec:int) -> str | None:
        ...
        with sd.RawInputStream(samplerate=ASR_SAMPLE_RATE, blocksize=8000, dtype='int16', channels=1, callback=cb):
            start=time.time()
...
class Assistant:
    ...
    def run(self):
        print('=== Medication Voice Assistant (diag) ===')
