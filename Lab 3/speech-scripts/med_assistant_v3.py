...
# -*- coding: utf-8 -*-
"""
Medication Voice Assistant (v3, Pillow 10+ safe)
- Fixes ImageDraw API change: uses textbbox() when available, falls back to textsize()
- Keeps PiTFT pins aligned with your lab (cs=D5, dc=D25, rst=None, backlight=GPIO22)
- Buttons on BCM23/24 via gpiozero; falls back to evdev or keyboard
- ASCII-safe TTS/logs

Suggested quick run while debugging display:
  USE_DISPLAY=0 BUTTON_BACKEND=gpio TOP_BUTTON_PIN=23 BOTTOM_BUTTON_PIN=24 \
  \
  "../.venv/bin/python" med_assistant_v3_fix.py

When ready to use the screen, stop the course service first:
  sudo systemctl stop piscreen.service --now
  BUTTON_BACKEND=gpio USE_DISPLAY=1 ../.venv/bin/python med_assistant_v3_fix.py
"""

import os, sys, time, re, queue, threading, subprocess
from datetime import datetime, timedelta

# Try to make stdout UTF-8; still keep ASCII-only strings
try:
...
# ---- Display ----
class Display:
    # Per your lab example: cs=D5, dc=D25, rst=None; portrait panel 135x240, we rotate 90 degrees
    ...
    def __init__(self):
        ...
        try:
            ...
            reset_pin = None  # avoid GPIO24 conflict with bottom button
            spi = board.SPI()
            ...
            # Create image in rotated orientation (we pass rotation each frame)
            self._image = Image.new('RGB', (self.HEIGHT, self.WIDTH))  # swapped
    ...
    def _measure_text(self, text:str):
        """Return (w, h) using Pillow 10+ safe API."""
        ...
        if hasattr(self._draw, 'textsize'):
    ...
    def _write_center(self, lines, fill=(255,255,255)):
        # Always log to console (ASCII-safe)
        for ln in lines:
...
class Assistant:
    ...
    def run(self):
        print('=== Medication Voice Assistant (v3-fix) ===')
