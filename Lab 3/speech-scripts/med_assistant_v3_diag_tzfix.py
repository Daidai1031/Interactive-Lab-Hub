...
# -*- coding: utf-8 -*-
"""
Medication Voice Assistant — DIAG BUILD (timezone-safe + "one minute" parsing)
- FIX 1: Avoids dateparser TypeError when TIMEZONE is None. We only pass
         TIMEZONE when a string is available; otherwise omit it.
- FIX 2: Understands bare relative phrases like "one minute", "30 seconds"
         even without the keyword "in/after".
- Keeps: Pillow 10+ safe text rendering, bytes(indata) for Vosk, GPIO23/24 buttons.

Quick run (no display):
  USE_DISPLAY=0 BUTTON_BACKEND=gpio GPIOZERO_PIN_FACTORY=lgpio \
  "../.venv/bin/python" med_assistant_v3_diag_tzfix.py

Use a specific mic (e.g., Logitech):
  SD_INPUT_NAME=Logi USE_DISPLAY=0 BUTTON_BACKEND=gpio GPIOZERO_PIN_FACTORY=lgpio \
  ../.venv/bin/python med_assistant_v3_diag_tzfix.py

With display (ensure you stopped the course service):
...
class Listener:
    def __init__(self, model_dir:str):
        ...
        # pick input device by name if asked
...
def parse_time_expr(utter:str):
    ...
    now=now_tz()
    # Case A: explicit relative keyword
    ...
    # Case B: bare relative like "one minute", "30 sec"
    bare_seconds=_parse_rel(utter)
    if bare_seconds:
        bare_seconds=clamp_delay(bare_seconds)
        return now+timedelta(seconds=bare_seconds), humanize_delta(bare_seconds)
    # Case C: absolute time like "at 8:05 pm"
    ...
    # Case D: fall back to dateparser with timezone-safe settings
...
class Assistant:
    ...
    def run(self):
        print('=== Medication Voice Assistant (diag-tzfix) ===')
    ...
    def _capture_and_schedule(self):
        ...
        if re.search(r"\b(in|after)\b", (utter or ''), re.I) or _parse_rel(utter or ''):
            ...
            at_str = dt.astimezone(ZoneInfo(LOCAL_TZ_NAME)).strftime('%I:%M %p').lstrip('0') if (ZoneInfo and dt.tzinfo) else dt.strftime('%H:%M')
